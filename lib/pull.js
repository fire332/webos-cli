...
(function () {
  ...
  const pull = {
    pull: function (srcPaths, dstPath, options, next, middleCb) {
                  ...
                  if (exists) {
                    if (stats.isDirectory()) {
                      destinationPath = path.resolve(
                        destinationPath,
                        path.basename(orginalSourcePath),
                      );
                    }
                  }
                } catch (err) {
                ...
                session.get(sourcePath, destinationPath, function (err) {
                  if (err) {
                    ...
                    if (!options.ignore) {
                      spinner.stop();
                      middleCb(
                        "Pull: " + sourcePath + " -> " + destinationPath,
                      );
                    }
              ...
              session.run(cmd, null, writeIsDir, null, function () {
                    ...
                    async.eachSeries(
                      ...
                      function (item, callback) {
                        const filepath = path.join(
                          destinationPath,
                          item.substring(dirIndex),
                        );
                        mkdirp(filepath, function (err) {
                          if (
                            !options.ignore &&
                            path.resolve(filepath) !== destinationPath
                          ) {
                            spinner.stop();
                ...
                if (writeIsDir.getContentsAsString() === "d\n") {
                  ...
                  try {
                    ...
                    if (!stat.isDirectory()) {
                      return next(
                        errHndl.getErrMsg("NOT_DIRTYPE_PATH", destinationPath),
                      );
                    }
                  } catch (e) {
                    if (e && e.code === "ENOENT") {
                  ...
                  async.waterfall(
                    [_copyAllFolders, _copyAllFiles],
                    function (err, result) {
                      setImmediate(next, err, result);
                    },
                  );
                } else {
                  const err = errHndl.getErrMsg(
                    "NOT_EXIST_PATH",
                    "SOURCE",
                    sourcePath,
                  );
      ...
      function finish(err, result) {
        ...
        if (!err) {
          ...
          if (!options.silent) {
            ...
            middleCb(
              Math.round(totalSize / (1024 * timeDur)) +
                " KB/s (" +
                totalSize +
                " bytes in " +
                timeDur +
      ...
      async.waterfall([_makeSession, _transferFiles], function (err, result) {
        finish(err, result);
      });
    },
  };
