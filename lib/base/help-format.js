...
(function () {
      ...
      data.display.forEach(function (prop) {
        ...
        if (data.notab && data.notab.indexOf(prop) !== -1) {
          helpString = helpString.concat(
            helpFormat.stringFormat(data[prop], false, hiddenFlag),
          );
        } else {
          helpString = helpString.concat(
            helpFormat.stringFormat(data[prop], true, hiddenFlag),
          );
        }
      });
  ...
  helpFormat.optionObject = function (data, tab) {
    let helpString = [],
      optionList = []
        .concat(data.default || [])
        .concat(data.default - process || []);
    optionList = optionList
      .concat(data[this.profile] || [])
      .concat(data[process.platform] || []);
    for (const index in optionList) {
      helpString = helpString.concat(
        helpFormat.stringFormat(data[optionList[index]], tab),
      );
    }
    return helpString;
  };
  ...
  helpFormat.stringObject = function (data, tab) {
    ...
    let helpString = [];
    helpString = helpString.concat(
      helpFormat.stringFormat(defaultData.concat(currentData), tab),
    );
    return helpString;
  };
  ...
  helpFormat.stringFormat = function (data, tab, hiddenFlag) {
    ...
    if (Array.isArray(data)) {
      ...
      if (data.cmdOpt && data.cmdOpt === "option") {
        ...
        helpString = helpString.concat(
          helpFormat.hiddenObject(data, tab, hiddenFlag),
        );
      } else {
