...
function launchHostedApp() {
  const pkgId =
    appdata.getConfig(true).profile === "signage"
  ...
  if (options.hostIp) {
    if (options.hostIp === "true") {
      ...
      const ipFormat =
        /^(?:(?:\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){3}(?:\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/gm;
...
function running() {
  launchLib.listRunningApp(options, function (err, runningApps) {
    ...
    if (runningApps instanceof Array)
      runningApps.forEach(function (runApp) {
        if (cnt++ !== 0) {
...
function launchSimulator() {
  ...
  if (curConfigData.profile !== "tv") {
    return finish(
      errHndl.getErrMsg("NOT_SUPPORT_OPTION", curConfigData.profile),
    );
  }
