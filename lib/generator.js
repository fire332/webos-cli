...
Generator.prototype.generate = function (options, next) {
  ...
  if (!template) {
    return next(
      errHndl.getErrMsg("INVALID_VALUE", "TEMPLATE", options.tmplName),
    );
  }
    ...
    return promise
      .all(
        filePaths.map(function (file) {
          return fs.lstatAsync(file).then(function (stats) {
            if (!stats.isFile()) {
          ...
          return fs
            ...
            .then(function (stats) {
              ...
              if (fileName === "appinfo.json") {
                ...
                (metaTmpl.type === "jsserviceinfo" ||
                  metaTmpl.type === "nativeserviceinfo")
              ) {
            ...
            .then(function (info) {
              const destFile = path.join(dest, path.basename(file));
              return fs.mkdirsAsync(dest).then(function () {
