...
function commandList() {
  ...
  try {
    const commandsList = JSON.parse(
      fs.readFileSync(
        path.join(__dirname, "../", "files", "conf", "ares.json"),
      ),
    );
    Object.keys(commandsList).forEach(function (cmd) {
      if (
        commandsList[cmd].profile &&
        !commandsList[cmd].profile.includes(profile)
      ) {
        return;
...
function display(next) {
  ...
  try {
    ...
    for (const arg in argv) {
      if (
        Object.hasOwnProperty.call(commandsList, "ares-" + arg) &&
        fs.existsSync(path.join(__dirname, "ares-" + arg + ".js"))
      ) {
        if (
          commandsList["ares-" + arg].profile &&
          !commandsList["ares-" + arg].profile.includes(profile)
        ) {
