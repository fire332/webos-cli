...
function PalmPackage() {
  ...
  this.hiddenhelpString = [
    ...
    help.format(
      "-f, --force",
      "Make .ipk package forcibly with same file structure in APP_DIR",
    ),
    help.format(
      "",
      "If file/directories in APP_DIR consists of the following structure",
    ),
  ...
  if (this.argv.level) {
    ...
    if (
      this.argv.argv.remain.length === 0 &&
      Object.keys(this.argv).length === 1
    ) {
      this.argv.help = true;
    }
  }
...
PalmPackage.prototype = {
  ...
  checkInputDir: function (next) {
    ...
    this.dirCounts = packager.checkInputDirectories(
      this.argv.argv.remain,
      this.options,
      next,
    );
  },
  ...
  packageApp: function (next) {
    log.info("packageApp()");
    const packager = new packageLib.Packager();
    if (this.dirCounts.app === 0) {
      // packaging without app
      if (
        Object.prototype.hasOwnProperty.call(this.options, "pkginfofile") &&
        Object.prototype.hasOwnProperty.call(this.options, "pkgid")
      ) {
        this.finish(
  ...
  checkApplication: function () {
    async.series(
      [version.checkNodeVersion, this.checkInputDir.bind(this)],
      function (err) {
        if (err) {
