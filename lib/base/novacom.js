...
const async = require("async"),
  ...
  log = require("npmlog"),
  path = require("path"),
  request = require("../util/fetch-request"),
...
(function () {
  ...
  function makeExecError(cmd, code, signal, orgErrMsg) {
    ...
    if (code !== 0 || signal) {
      err = new Error();
      err.message =
        "Command '" + cmd + "' exited with code=" + code + signal + orgErrMsg;
      err.code = code;
  ...
  novacom.Resolver.prototype = {
    ...
    load: function (next) {
      ...
      function _replaceBuiltinSshKey(next) {
        ...
        const builtinPrvKeyForEmul = path.join(
            __dirname,
            "../../files/conf/",
            "webos_emul",
          ),
          ...
          if (err) {
            ...
            fs.stat(userHomePrvKeyForEmul, function (error, userKeyStat) {
              if (error) {
                ...
                if (
                  builtinKeyStat.mtime.getTime() > userKeyStat.mtime.getTime()
                ) {
                  shelljs.cp("-rf", builtinPrvKeyForEmul, keydir);
                  fs.chmodSync(userHomePrvKeyForEmul, "0600");
                }
      ...
      function _adjustList(next) {
        ...
        if (defaultFields.length < 1) {
          log.silly(
            "novacom#Resolver()#load()#_adjustList()",
            "rewrite default field",
          );
    ...
    _loadOne: function (inDevice, next) {
      if (typeof inDevice.privateKey === "string") {
        ...
        typeof inDevice.privateKey === "object" &&
        typeof inDevice.privateKey.openSsh === "string"
      ) {
        ...
        async.waterfall(
          [
            fs.readFile.bind(
              this,
              path.join(keydir, inDevice.privateKey.openSsh),
              next,
            ),
          ...
          function (err) {
            ...
            if (err) {
              log.verbose(
                ...
                "Unable to find SSH private key named '" +
                  inDevice.privateKey.openSsh +
                  "' from '" +
                  keydir +
                  " for '" +
                  inDevice.name +
        ...
        setImmediate(
          ...
          errHndl.getErrMsg(
            "NOT_IMPLEMENTED",
            "webOS Pro device type handling",
          ),
        );
      } else {
        // private Key is not defined in novacom-device.json
        if (!inDevice.password) {
          log.silly(
            "novacom#Resolver()#_loadOne()",
            "Regist privateKey : need to set a SSH private key in " +
              keydir +
              " for'" +
              inDevice.name +
              "'",
          ...
          function (targetDevice, next) {
            ...
            request.head(url, function (err, res) {
              if (err || (res && res.statusCode !== 200)) {
                return setImmediate(
                  next,
                  errHndl.getErrMsg("FAILED_GET_SSHKEY"),
                );
              }
              log.info(
                "Resolver#getSshPrvKey()#head",
                "content-type:",
              ...
              request(url)
                .pipe(fs.createWriteStream(keySavePath))
    ...
    modifyDeviceFile: function (op, target, next) {
      ...
      if (op === "add") {
        if (matchedDevices.length > 0) {
          return setImmediate(
            next,
            errHndl.getErrMsg("EXISTING_VALUE", "DEVICE_NAME", target.name),
          );
        }
        ...
        if (matchedDevices.length === 0) {
          return setImmediate(
            next,
            errHndl.getErrMsg("INVALID_VALUE", "DEVICE_NAME", target.name),
          );
        }
          ...
          inDevices = inDevices.filter(function (dev) {
            if (target.name === dev.name) {
              if (dev.indelible === true) {
                return setImmediate(
                  next,
                  errHndl.getErrMsg("CANNOT_REMOVE_DEVICE", dev.name),
                );
              } else {
  ...
  function Session(target, printTarget, next) {
    ...
    log.info(
      "novacom#Session()",
      "opening session to '" + (name ? name : "default device") + "'",
    );
  ...
  novacom.Session.prototype = {
    ...
    begin: function (target, next) {
      ...
      if (
        this.target.privateKey === undefined &&
        this.target.password === undefined
      ) {
      ...
      if (!this.ssh) {
        ...
        this.ssh.on("close", function (had_error) {
          log.info(
            "novacom#Session()#begin()",
            "ssh session event: close (had_error:",
            had_error,
            ")",
          );
        });
      ...
      self.sftpPut(inPath, outPath, function (err) {
        if (err) {
          ...
          if (4 === err.code || 127 === err.code) {
            log.info(
              "novacom#Session()#put()",
              "sftp is not available, attempt transfering file via streamPut",
            );
    ...
    streamPut: function (outPath, inStream, next) {
      log.info(
        "novacom#Session()#streamPut()",
        "streaming into device:" + outPath,
      );
    ...
    sftpPut: function (inPath, outPath, next) {
      log.verbose(
        "novacom#Session()#sftpPut()",
        "host:" + inPath + " => " + "device:" + outPath,
      );
      const self = this;
      self.ssh.sftp(function (err, sftp) {
        ...
        // Wait for the SFTP channel close handshake to complete
        // before proceeding — dropbear rejects new channels if the
        // previous one is still mid-close.
    ...
    streamGet: function (inPath, outStream, next) {
      log.verbose(
        "novacom#Session()#streamGet()",
        "streaming from device:" + inPath,
      );
    ...
    sftpGet: function (inPath, outPath, next) {
      log.verbose(
        "novacom#Session()#sftpGet()",
        "target:" + inPath + " => " + "host:" + outPath,
      );
      const self = this;
      self.ssh.sftp(function (err, sftp) {
        if (err) {
    ...
    run_ssh: function (cmd, opt, stdin, stdout, stderr, next) {
      log.info(
        "novacom#Session()#run()",
        "cmd:" + cmd + ", opt:" + JSON.stringify(opt),
      );
      if (typeof next !== "function") {
        throw errHndl.getErrMsg("MISSING_CALLBACK", "next", util.inspect(next));
      }
      ...
      if (!stdout) {
        ...
        return setImmediate(
          next,
          errHndl.getErrMsg("INVALID_VALUE", "stdout", util.inspect(stdout)),
        );
      }
      ...
      if (!stderr) {
        ...
        return setImmediate(
          next,
          errHndl.getErrMsg("INVALID_VALUE", "stderr", util.inspect(stderr)),
        );
      }
      ...
      this.ssh.exec(cmd, opt, function (err, chStream) {
        log.silly(
          "novacom#Session()#run()",
          "exec cmd:" + cmd + ", opt:" + JSON.stringify(opt) + ", err:" + err,
        );
        if (err) {
          return setImmediate(next, err);
        }
    ...
    forward: function (devicePort, localPort, forwardName, next) {
      ...
      if (localPort !== 0) {
        if (
          session.forwardedPorts.indexOf({
            name: registerName,
            local: localPort,
            device: devicePort,
          }) > 0
        ...
        session.forwardedPorts.filter(function (forwardItem) {
          return (
            forwardItem.device === devicePort &&
            forwardItem.name === registerName
          );
        }).length > 0
      ) {
        forwardInUse = true;
      ...
      const localServer = net.createServer(function (inCnx) {
        log.info(
          "novacom#Session()#forward()",
          "new client, localPort:",
          localPort,
        );
        log.verbose(
          "novacom#Session()#forward()",
          "new client, from: " + inCnx.remoteAddress + ":" + inCnx.remotePort,
        );
        ...
        session.ssh.forwardOut(
          "127.0.0.1" /* srcAddr*/,
          inCnx.remotePort /* srcPort*/,
          "127.0.0.1" /* dstAddr*/,
          devicePort /* dstPort*/,
          function (err, outCnx) {
            if (err) {
              console.log(
                "novacom#Session()#forward()",
                "failed forwarding client localPort:",
                localPort,
                "(inCnx.remotePort:",
                inCnx.remotePort,
                ")=> devicePort:",
