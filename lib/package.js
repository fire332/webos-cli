...
(function () {
  ...
  Packager.prototype = {
    prepareOptions: function (options, next) {
      ...
      if (
        options &&
        Object.prototype.hasOwnProperty.call(options, "excludefiles")
      ) {
      ...
      if (
        options &&
        Object.prototype.hasOwnProperty.call(options, "remainPlainIPK")
      ) {
      ...
      if (
        options &&
        Object.prototype.hasOwnProperty.call(options, "certificate")
      ) {
      ...
      if (
        options &&
        Object.prototype.hasOwnProperty.call(options, "pkginfofile")
      ) {
      ...
      if (this.pkgId && this.pkginfofile) {
        return next(
          errHndl.getErrMsg("NOT_USE_WITH_OPTIONS", "pkginfofile, pkgid"),
        );
      }
      this.appCount = 0;
    },
    checkInputDirectories: function (inDirs, options, next) {
      log.verbose(
        "package#Packager#checkInputDirectories()",
        "input directory:",
        inDirs,
      );
      ...
      async.forEachSeries(
        inDirs,
        checkDirectory.bind(this, options),
        function (err) {
          if (err) {
    ...
    servicePackaging: function (inDirs, destination, options, middleCb, next) {
      ...
      if (
        !Object.hasOwnProperty.call(options, "pkgid") &&
        !Object.hasOwnProperty.call(options, "pkginfofile")
      ) {
    ...
    analyzeIPK: function (options, next) {
      ...
      async.series(
        [_setConfig, _unpackIpk, _unpackTar, _analyzeMetaFile, _removeTmpDir],
      ...
      function _unpackTar(next) {
        ...
        if (fs.existsSync(path.resolve(tmpDirPath, "control.tar.gz"))) {
          (async function () {
            await decompress(
              path.resolve(tmpDirPath, "control.tar.gz"),
              tmpDirPath,
              {
            ...
            log.verbose(
              "package#analyzeIPK()#_unpackTar()",
              "control.tar.gz decompressed",
            );
          })();
          ...
          return next(
            errHndl.getErrMsg("NO_COMPONENT_FILE", ", control tar file"),
          );
        }
        ...
        if (fs.existsSync(path.resolve(tmpDirPath, "data.tar.gz"))) {
          (async function () {
            await decompress(
              path.resolve(tmpDirPath, "data.tar.gz"),
              tmpDirPath,
              {
                plugins: [decompressTargz()],
              },
        ...
        ipkConfig.webOSMetaFiles.forEach(function (item) {
          ...
          if (item === "control") {
            ...
            if (!fs.existsSync(targetFile)) {
              return next(
                errHndl.getErrMsg("NO_COMPONENT_FILE", "control file"),
              );
            }
            ...
            if (options.infodetail) {
              ...
              ipkConfig[item].info.forEach(function (field) {
                if (tmpTxt.match(new RegExp(field + ": (.*)", "gi"))) {
                  result +=
                    tmpTxt.match(new RegExp(field + ": (.*)", "gi"))[0] + "\n";
                }
              });
            }
                      ...
                      ipkConfig[item].info.forEach(function (field) {
                        if (tmpJson[field]) {
                          ...
                          if (typeof fieldValue === "object") {
                            if (ipkConfig[item][field]) {
                              // Case of services.json, services_name field
                              ...
                              result +=
                                field + ": " + JSON.stringify(subResult) + "\n";
                            } else {
                              // Case of packageinfo.json, services field
                              result +=
                                field +
                                ": " +
    ...
    if (this.pkginfofile) {
      ...
      if (fs.existsSync(this.pkginfofile)) {
        if ("packageinfo.json" !== path.basename(this.pkginfofile)) {
          return setImmediate(
            next,
            errHndl.getErrMsg("INVALID_FILE", "packageinfo.json"),
          );
        }
        ...
        try {
          ...
          if (!Object.prototype.hasOwnProperty.call(this.pkginfo, "id")) {
            return setImmediate(
              next,
              errHndl.getErrMsg("REQUIRED_FIELD", "id"),
            );
          }
          ...
          return setImmediate(
            next,
            errHndl.getErrMsg("INVALID_JSON_FORMAT", "packageinfo.json"),
          );
        }
      } else {
        return setImmediate(
          next,
          errHndl.getErrMsg("NOT_EXIST_PATH", this.pkginfofile),
        );
      }
    } else if (this.pkgDir) {
  ...
  function checkAppInfo(next) {
    ...
    if (
      this.pkgJSExist &&
      this.appinfo.main &&
      this.appinfo.main.match(/(\.html|\.htm)$/gi)
    ) {
      const mainFile = path.join(this.appDir, this.appinfo.main);
      if (!fs.existsSync(mainFile)) {
        return setImmediate(
          next,
          errHndl.getErrMsg("NOT_EXIST_PATH", this.appinfo.main),
        );
      }
    ...
    if (
      this.appinfo.version.length < 1 ||
      !/^([1-9]\d{0,8}|\d)\.([1-9]\d{0,8}|\d)\.([1-9]\d{0,8}|\d)$/.test(
        this.appinfo.version,
      )
    ) {
      log.error(
        errHndl.getErrMsg("INVALID_VALUE", "version", this.appinfo.version),
      );
    ...
    const schemaFile = path.resolve(
      __dirname,
      "../files/schema/ApplicationDescription.schema",
    );
    async.waterfall(
      [
        ...
        function getSchema(data, next) {
          try {
            ...
            next(
              errHndl.getErrMsg("INVALID_JSON_FORMAT", "AppDescription schema"),
            );
          }
        },
        ...
        if (err) {
          ...
          if (result && result.errors.length > 0) {
            ...
            for (const idx in result.errors) {
              let errMsgLine =
                result.errors[idx].property + " " + result.errors[idx].message;
  ...
  function fillAssetsField(next) {
    ...
    for (const i in this.appinfo) {
      if (
        Object.prototype.hasOwnProperty.call(this.appinfo, i) &&
        defaultAssetsFields[i]
      ) {
        // no duplicated adding & value should not null string & file/dir should exist
        if (
          this.appinfo.assets.indexOf(this.appinfo[i]) === -1 &&
          this.appinfo[i]
        ) {
              ...
              rewriteFileWoBOMAsUtf8(filePath, true, function (err, data) {
                try {
                  ...
                  for (const i in appInfo) {
                    if (
                      Object.prototype.hasOwnProperty.call(appInfo, i) &&
                      defaultAssetsFields[i]
                    ) {
                      if (appInfo[i]) {
                  ...
                  setImmediate(
                    next,
                    errHndl.getErrMsg("INVALID_JSON_FORMAT", filePath),
                  );
                }
              });
            },
  ...
  function createTmpDir(next) {
    this.tempDir =
      temp.path({ prefix: "com.palm.ares.hermes.bdOpenwebOS" }) + ".d";
  ...
  function createAppDir(next) {
    ...
    try {
      this.applicationDir = path.resolve(
        this.tempDir,
        "data/usr/palm/applications",
        this.appinfo.id,
      );
      log.info(
        "package#createAppDir()",
        ...
        async.forEachSeries(
          ...
          function (file, next) {
            if (!FILE_TYPE[file.type]) {
              log.verbose(
                "package#copySrcToDst()#_copySrcToDst()",
                "ignore 'unknown file type'(" + file.type + ")",
              );
              return;
            }
            ...
            if (file.type === FILE_TYPE.symlink) {
              ...
              if (fs.existsSync(sourceFile)) {
                if (
                  minify &&
                  ".js" === path.extname(sourceFile) &&
                  file.relPath.indexOf("node_modules") === -1
                ) {
                  log.verbose(
                    "package#copySrcToDst()#_copySrcToDst()",
                    "require minification # sourceFile:",
                    sourceFile,
                  );
                  try {
                    const data = uglify.minify(
                    ...
                    fs.writeFileSync(
                      path.join(dstPath, file.relPath),
                      data.code,
                      "utf8",
                    );
                  } catch (e) {
                    ...
                    return setImmediate(
                      next,
                      errHndl.getErrMsg("FAILED_MINIFY", sourceFile),
                    );
                  }
                } else {
                  shelljs.cp(
  ...
  function checkELFHeader(next) {
    ...
    if (stats.size < ELF_HEADER_LEN) {
      log.verbose(
        "package#checkELFHeader()",
        "file size is smaller than ELF Header size",
      );
      return setImmediate(next);
    }
    ...
    fs.read(fd, buf, 0, ELF_HEADER_LEN, 0, function (err, bytesRead, _buf) {
      if (bytesRead < ELF_HEADER_LEN || err) {
        ...
        log.silly(
          "package#checkELFHeader()",
          "readBuf to parse ELF header is small or error occurred during reading file.",
        );
        return setImmediate(next);
      }
  ...
  function copyApp(next) {
    ...
    log.info(
      "package#copyApp()",
      "copy " + this.appDir + " ==> " + this.applicationDir,
    );
  ...
  function copyAssets(next) {
    ...
    function _handleAssets(file, next) {
      if (path.resolve(this.originAppDir) === path.resolve(this.appDir)) {
        ...
        async.series(
          [_checkAppinfo.bind(this, file), _copyAssets.bind(this, file)],
          next,
        );
      }
    }
    ...
    function _copyAssets(file, next) {
      log.verbose(
        "package#copyAssets()#_copyAssets",
        "'" + file + "' will be located in app directory",
      );
  ...
  function excludeFromApp(middleCb, next) {
    ...
    async.series(
      [
        _retrieve.bind(this, excludeList, regExp, tempDirPrefix, 0),
        function (next) {
          const meta_files = [
              "appinfo.json",
              "services.json",
  ...
  function loadPackageProperties(inDirs, next) {
    ...
    async.forEach(
      ...
      function (err) {
        ...
        self.excludeFiles = self.excludeFiles.concat(["package.properties"]);
        setImmediate(next, err);
      },
    );
  }
  ...
  function decidePkgName(pkgName, pkgVersion, next) {
    if (this.appCount !== 0) {
      ...
      this.pkg = {
        name:
          pkgName ||
          this.services[0].serviceInfo.id ||
          this.services[0].serviceInfo.services[0].name,
  ...
  function createDataHash(dstDir, dataTgzPath, next) {
    ...
    log.verbose(
      "package#createDataHash()",
      "dataTgzPath : " + dataTgzPath + ", signfile : " + signFilePath,
    );
    ...
    try {
      ...
      readStream.on("readable", function () {
        ...
        if (data) {
          ...
          log.verbose(
            "package#createDataHash()",
            "Data.tar.gz Hash : " + self.dataHash,
          );
          setImmediate(next);
        }
      });
  ...
  function createCertificateHash(dstDir, next) {
    ...
    try {
      if (!self.certificate) {
        log.verbose(
          "package#createCertificateHash()",
          "Including certificate skipped",
        );
        return setImmediate(next);
      }
      ...
      fs.readFile(crtPath, function (err, certData) {
        ...
        log.verbose(
          "package#createCertificateHash()",
          "Certificate Hash : " + self.certificateHash,
        );
  ...
  function createControlFile(dstDir, encInfo, next) {
    ...
    log.verbose(
      "package#createControlFile()",
      "createControlFile:" + dstFilePath,
    );
  ...
  function createControlSignFile(ctrlDir, next) {
    ...
    try {
      if (this.sign === null) {
        log.verbose(
          "package#createControlSignFile()",
          "App signing is skipped",
        );
        return setImmediate(next);
      }
      ...
      log.verbose(
        "package#createControlSignFile()",
        "controlSignFilePath: " + controlSignFilePath,
      );
  ...
  function createSign(dstDir, dataTgzPath, next) {
    ...
    log.verbose(
      "package#createSign()",
      "dataTgzPath:" + dataTgzPath + ", sigfile:" + sigFilePath,
    );
    log.verbose(
      "package#createSign()",
      "keyPath:" + keyPath + ", crtPath:" + crtPath,
    );
  ...
  function makeTgz(srcDir, dstDir, next) {
    ...
    const fixupDirs = function (filePath, entry) {
      ...
      if (fileOrFolderName.length !== Buffer.byteLength(fileOrFolderName)) {
        const errMsg =
            "Please use the file name in english letters. \n\t\t (" +
            filePath +
            ")",
  ...
  function setIpkFileName(next) {
    ...
    if (this.pkg.version) {
      ...
      const archSuffix =
        "i586" === this.architecture ? "x86" : this.architecture || "all";
      filename = filename.concat(
        "_" + this.pkg.version + "_" + archSuffix + ".ipk",
      );
    } else {
  ...
  function makeIpk(srcDir, next) {
    ...
    log.info(
      "package#makeIpk()",
      "makeIpk in dir " + this.IpkDir + " file " + this.ipkFileName,
    );
    ...
    if (this.nativecmd) {
      ...
      shelljs.exec(
        "ar -q " + this.ipk + " debian-binary control.tar.gz data.tar.gz",
        { silent: this.silent },
      );
  ...
  function checkDirectory(options, directory, callback) {
    ...
    if (fs.existsSync(path.join(directory, "appinfo.json"))) {
      ...
      this.appCount++;
      log.verbose(
        "package#checkDirectory()",
        "FOUND appinfo.json, appCount " + this.appCount,
      );
      ...
      findServiceDir.call(this, foundSvcDirs, function () {
        if (foundSvcDirs.length > 0) {
          ...
          callback(
            errHndl.getErrMsg("NO_METAFILE", "APP_DIR/SVC_DIR", directory),
          );
        }
      });
    }
  ...
  function loadServiceInfo(next) {
    ...
    log.info(
      "package#loadServiceInfo()",
      "num of serviceInfo: " + this.services.length,
    );
    setImmediate(next);
  }
  ...
  function createServiceDir(next) {
    this.services.forEach(
      function (service) {
        ...
        getPkgServiceNames(service.serviceInfo).forEach(
          function (serviceName) {
            const serviceDir = path.join(
              this.tempDir,
              "data/usr/palm/services",
              serviceName,
            );
            service.dstDirs.push(serviceDir);
            try {
              log.info(
                "package#createServiceDir()",
  ...
  function addServiceInPkgInfo(next) {
    if (!this.rom) {
      ...
      if (validServiceCount > 0) {
        ...
        log.silly(
          "package#addServiceInPkgInfo()",
          "Modified package.json:" + data,
        );
        fs.writeFile(
          path.join(this.packageDir, "packageinfo.json"),
          data,
          next,
        );
      } else {
  ...
  function loadResourceInfo(next) {
    ...
    this.resources.forEach(function (resource) {
      ...
      try {
        ...
        return setImmediate(
          next,
          errHndl.getErrMsg("INVALID_JSON_FORMAT", "resourceinfo.json"),
        );
      }
    });
  ...
  function checkResourceInfo(next) {
    ...
    this.resources.forEach(function (resource) {
      if (!resource.info.id) {
        ...
        return setImmediate(
          next,
          errHndl.getErrMsg("INVALID_RESOURCEID", pkgId),
        );
      } else if (
        resource.info.id.length < 1 ||
        !/^[a-z0-9.+-]*$/.test(resource.info.id)
      ) {
    ...
    try {
      async.forEachSeries(
        ...
        function (resource, next) {
          self.dataCopyCount++;
          log.verbose(
            "copyResource(), copy " + resource.dir + " ==> " + resource.dstDir,
          );
  ...
  function getPkgServiceNames(serviceInfo) {
    ...
    if (servicePkgMethod === "id") {
      ...
      const serviceProps =
        serviceInfo.services instanceof Array
  ...
  function rewriteFileWoBOMAsUtf8(filePath, rewriteFlag, next) {
    ...
    if (["UTF-8", "ISO-8895-1"].indexOf(encodingFormat) === -1) {
      log.silly(
        "package#rewriteFileWoBOMAsUtf8()",
        "current encoding type:" + encodingFormat,
      );
  ...
  function encryptPackage(next) {
    if (this.rom || !this.encrypt) {
      // Do not encrypt when -rom option is given
      setImmediate(next);
      return;
    } else {
  ...
  function copyOutputToDst(destination, middleCb, next) {
    ...
    if (this.rom) {
      ...
      if (this.remainPlainIPK) {
        log.verbose(
          "Remain Plain IPK file from : " + self.tempDir,
          self.plainIpkFileName,
        );
        log.verbose("                        to : " + destination);
        middleCb(
          "Remain Plain IPK file " +
            self.plainIpkFileName +
            " to " +
            destination,
        );
        copySrcToDst.call(
          self,
          path.join(self.tempDir, self.plainIpkFileName),
          destination,
          function () {
            middleCb(
              "Create encrypted " + self.ipkFileName + " to " + destination,
            );
            copySrcToDst.call(
              self,
              path.join(self.encryptDir, self.ipkFileName),
              destination,
              next,
            );
          },
        ...
        copySrcToDst.call(
          this,
          path.join(this.encryptDir, this.ipkFileName),
          destination,
          next,
        );
      }
      ...
      copySrcToDst.call(
        this,
        path.join(this.tempDir, this.ipkFileName),
        destination,
        next,
      );
    }
  ...
  function encryptIpk(dstPath, next) {
    log.verbose(
      "package#encryptPackage()#encrypIpk()",
      "encrypt plain ipk to /encrypt/data",
    );
    ...
    try {
      ...
      output
        .on("close", function () {
          log.verbose(
            "package#encryptPackage()#encrypIpk()",
            "encrypted Ipk to " + encrypedIpkPath,
          );
          if (self.remainPlainIPK === true) {
            log.verbose(
              "Remain plain IPK file name  : " + path.join(plainIpkPath),
            );
            log.verbose(
              "**                       to : " +
                path.join(self.tempDir, self.plainIpkFileName),
