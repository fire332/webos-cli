...
(function () {
  ...
  const inspector = {
    ...
    inspect: function (options, next) {
      ...
      if (
        options &&
        Object.prototype.hasOwnProperty.call(options, "serviceId")
      ) {
      ...
      async.series(
        ...
        function (err, results) {
          ...
          returnObj.msg = options.serviceId
            ? results[6]
            : "Application Debugging - " + results[5];
          next(err, returnObj);
        },
      );
      ...
      function _makeSession(next) {
        ...
        options.session = new novacom.Session(
          options.device,
          printTarget,
          next,
        );
      ...
      function _runAppPortForwardServer(next) {
        if (options.appId) {
          ...
          log.verbose(
            "inspect#inspect()#_runAppPortForwardServer()",
            "insptPort : " + insptPort,
          );

          options.session.forward(
            insptPort,
            options.hostPort || 0 /* random port */,
            options.appId,
            next,
          );
        } else {
      ...
      function __findNewDebugPort(dbgPort, next) {
        ...
        function _onData(data) {
          ...
          if (str === "0") {
            log.silly(
              "inspect#inspect()#__findNewDebugPort()",
              "final dbgPort : " + dbgPort,
            );
            next(null, dbgPort);
          } else if (str === "1") {
      ...
      function __getNodeVersion(next) {
        ...
        function _onData(data) {
          ...
          platformNodeVersion = Number(
            parsedVersion[0].substring(1, parsedVersion[0].length),
          );
          next();
        }
      }
      ...
      function _runServicePortForwardServer(next) {
        ...
        function __eachServicePortForward(serviceId, next) {
          ...
          log.info(
            "inspect#inspect()#_eachServicePortForward()",
            "sessionId:" + options.sessionId + ", default dbgPort : " + dbgPort,
          );
          const __printInspectGuide = function (svcId, next) {
            ...
            guideText +=
              "To debug your service, set " +
              '"127.0.0.1:' +
              options.session.getLocalPortByName(svcId) +
            ...
            request.get(nodeInsptUrl, function (error, response) {
              ...
              function _postAction(err, serverInfo) {
                if (err) {
                  ...
                  server.openBrowser(
                    serverInfo.openBrowserUrl,
                    options.bundledBrowserPath,
                  );
                }
            ...
            [
              function findSvcFilePath(next) {
                ...
                if (options.instPkgs) {
                  options.instPkgs.every(function (pkg) {
                    if (serviceId.indexOf(pkg.id) !== -1) {
                      options.svcDbgInfo[serviceId].path = path
                        .join(
                          path.dirname(pkg.folderPath),
                          "..",
                          "services",
                          serviceId,
                        )
                ...
                if (!options.svcDbgInfo[serviceId].path) {
                  return next(
                    errHndl.getErrMsg("FAILED_GET_SVCPATH", serviceId),
                  );
                }
                next();
              },
              function parserMeta(next) {
                const metaFilePath = path
                    .join(options.svcDbgInfo[serviceId].path, "services.json")
                    .replace(/\\/g, "/"),
                ...
                async.series(
                  ...
                  function (err) {
                    if (err) {
                      return next(
                        errHndl.getErrMsg("FAILED_FIND_SERVICE", serviceId),
                      );
                    }
                  },
                );
              ...
              function doPortForward(next) {
                log.info(
                  "inspect#inspect()#_doPortForward()",
                  "platformNodeVersion: " +
                    platformNodeVersion +
                    ", nodeBaseVersion: " +
                    nodeBaseVersion,
              ...
              function clearDbgFile(next) {
                const cmdRmDbgFile =
                  "rm -rf " + options.svcDbgInfo[serviceId].path + "/_ares";
      ...
      function _runAppInspector(next) {
        let url,
          ...
          listFiles = [
            {
              reqPath: "/pagelist.json",
              propName: "inspectorUrl",
            } /* AFRO, BHV */,
            {
              reqPath: "/json/list",
              propName: "devtoolsFrontendUrl",
            } /* DRD */,
          ];
        ...
        function _getDisplayUrl(next) {
          ...
          request.get(url + listFile.reqPath, function (error, response, body) {
            ...
            for (const index in pagelist) {
              if (
                pagelist[index].url.indexOf(options.appId) !== -1 ||
        ...
        if (options.appId) {
          url =
            "http://localhost:" +
            options.session.getLocalPortByName(options.appId);
