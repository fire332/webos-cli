...
(function () {
  const errMsgHdlr = {},
    ErrMsgMap = {
      ...
      // appinstalld
      "failed to extract ipk file":
        "Please check whether the ipk file is packaged by CLI or not",
      ...
      "Cannnot install privileged app on developer mode":
        "Please change the app id (app id should not start with 'com.lge', 'com.webos', 'com.palm')",
      "Cannnot remove privileged app on developer mode":
        "You cannot remove the privileged app whose app id start with 'com.lge', 'com.webos', 'com.palm'",
      "unable to execute smack": "Please remove the app and install again",
      "empty session list, cannot query getAppInfo":
        "Please select the profile on the display",
      FAILED_REMOVE:
        "Please check app is installed to device by ares-install -l",

      // applicationmanager
      "Cannot find proper launchPoint":
        "The app is not installed app. Please check the list by ares-install -l",
      "is not running":
        "Please check the list of running apps using ares-launch -r",
      "was not found OR Unsupported Application Type":
        "The app is not installed. Please check the list of installed apps using ares-install -l",
      "invalid parameters":
        "Invalid parameters are passed. Please check the parameters",
      "app is locked":
        "The app is now installing/updating/deleting. Please try again later",

      // com.webos.service.sessionmanager
      "Service does not exist: com.webos.service.sessionmanager":
        "This device does not support multiple sessions",
      ...
      EADDRINUSE:
        "An attempt to bind a server to a local address failed. Please check the IP address or the port number",
      ECONNREFUSED:
        "Connection refused. Please check the device IP address or the port number",
      ...
      NO_FREE_SPACE:
        "Installation failure. Please check if there is sufficient free space in the disk",

      // can be external lib error
      "Authentication failure":
        "ssh authentication failure. Please check the ssh connection info such as password, private key and username again",
      "connect Unknown system":
        "Please check the device IP address or the port number",
      "Unable to parse private key":
        "Wrong passphrase for ssh key. Please check passphrase again",
      "Unable to request a pseudo-terminal":
        "Unable to open terminal (Target does not allow to open pty)",

      // invalid appinfo, service
      REQUIRED_FIELD: "Please input required field",
      INVALID_ID_RULE:
        "The app/pkg ID should consist of lowercase letters (a-z), numbers (0-9), plus (+) and minus (-) signs and periods (.)",
      INVALID_VERSION_RULE:
        "The app/pkg version number should consist of three non-negative integers, separated by dots.\nEach number cannot exceed 9 digits and cannot contain leading zeroes",
      NOT_EXCLUDE_APPINFO: "You cannot exclude the appinfo.json file",
      NOT_RELATIVE_PATH_APPINFO:
        "The pathname must be relative to the directory where the appinfo.json file is located",
      NO_METAFILE:
        "No meta file (ex. appinfo.json, services.json) exists in the project directory",
      NO_COMPONENT_FILE: "There is no component file for webOS package. File:",
      NO_ACCOUNT:
        "Account directory is not supported. Please delete the 'account-templates.json' file.",
      ...
      INVALID_INTERVAL:
        'Please use nonnegative integer value for the "time-interval" option',
      INVALID_DISPLAY:
        'Please use nonnegative integer value for the "display" option',
      INVALID_TEMPLATE: "Invalid template name",
      INVALID_FILE: "Invalid file",
      INVALID_VALUE: "Invalid value",
      INVALID_DEVICENAME:
        "Invalid device name. The device name should not start with '%' or '$'",
      ...
      INVALID_CAPTURE_FORMAT:
        "Please specify the file extension(.png, .bmp or .jpg)",
      INVALID_CSV_FORMAT: "Please change the file extension to .csv",
      INVALID_IP: "Plese check IP address format",
      INVALID_COMMAND:
        "This command is invalid. Please check the supported commands using ares -l",
      ...
      EXISTING_FILETYPE_PATH:
        "A file with the same name at the specified location already exists",
      SUPPORT_ONLY_IPK:
        "Only .ipk file is supported. Please check the file extension",
      NOT_MATCHED_LOGDAEMON:
        "Logging daemon of CLI should be matched with that of the target device\nPlease change the logging daemon of CLI to same as that of the target device",
      NOT_MATCHED_LOG:
        "There are no logs from the ID\nPlease check if the combination of options or the ID are valid",
      NOT_DIRTYPE_PATH: "The specified path is not a directory",
      NOT_EXIST_INSPECTOR:
        "Inspector is not running. Please call stop() after inspect the app",
      ...
      NOT_EXIST_SERVER:
        "Server is not running. Please call stop() after running the server",
      ...
      NOT_EXIST_SIMULATOR:
        "No existing simulator excutable file which name starting with",
      NOT_SUPPORT_COMMOND_DEPRECATED:
        'This command is deprecated. Please use "ares-device -i" instead of "ares-device-info"',
      NOT_SUPPORT_COMMOND:
        'This command is not supported by current profile\nIf you want to use this command, please change profile using "ares-config" command\nCurrent profile:',
      NOT_SUPPORT_OPTION:
        'This option is not supported by current profile\nIf you want to use this command, please change profile using "ares-config" command\nCurrent profile:',
      ...
      NOT_SUPPORT_RUNNINGLIST:
        "Not supported method to get running app information",
      NOT_SUPPORT_JOURNALD: "journald does not support the option",
      NOT_SUPPORT_PMLOGD: "pmlogd does not support the option",
      NOT_SUPPORT_XZ:
        "xz file is not supported. Only .ipk file packaged with the CLI is supported",
      ...
      USE_PKGID_PKGINFO:
        "packageId must be provided by using either the '--pkgid' or the '--pkginfofile' option",
      ...
      NOT_PACKAGE_WITH_PKGDIR:
        "Cannot package the directory with package directory",
      NOT_PACKAGE_WITH_RESOURCE:
        "Cannot package the app or service directory with resource directory",
      UNMATCHED_DEVICE: "No matched device",
      CANNOT_REMOVE_DEVICE: "Cannot remove the device",
      UNMATCHED_DISPLAY_AFFINITY:
        'Please use the same value for "display" and "displayAffinity"',
      ...
      FAILED_CREATE_DIR:
        "Cannot create directory in the destination path - permission denied",
      ...
      CONNECTED_MULTI_DEVICE:
        "Multiple devices are connected by novacom. Please specify the target name",
      SET_DEFAULT_MULTI_DEVICE:
        "Multiple devices are set to default target. Please reset device list",
      NEED_ROOT_PERMISSION:
        "Unable to connect to the target device. root access required",
      MISSING_CALLBACK: "Missing completion callback",
      USING_WEBINSPECTOR:
        "Web Inspector is already connected with another browser. Please close the previous connection",
