  ...
  const installer = {
    ...
    install: function (options, hostPkgPath, next, middleCb) {
      ...
      const appId = "com.ares.defaultName",
        devicePkgPath = path
          .join(config.tempDirForIpk, hostPkgName)
          .replace(/\\/g, "/"),
        ...
        [
          ...
          function (session, next) {
            if (options.opkg) {
              ...
              if (options.session.getDevice().username !== "root") {
                return setImmediate(
                  next,
                  errHndl.getErrMsg("NEED_ROOT_PERMISSION", "opkg install"),
                );
              }
            }
            let cmd =
              "/usr/bin/test -d " +
              config.tempDirForIpk +
              " || /bin/mkdir -p " +
              config.tempDirForIpk;
            if (options.session.getDevice().username === "root") {
              ...
              [
                ...
                function () {
                  ...
                  if (!srcMd5) {
                    log.warn(
                      "install#install()",
                      "Failed to get md5sum from the ipk file",
                    );
                  }
          ...
          function (next) {
            ...
            function _onData(data) {
              ...
              if (!dstMd5) {
                log.warn(
                  "install#install()",
                  "Failed to get md5sum from the transmitted file",
                );
              }
              next();
            }
          ...
          function (next) {
            ...
            function _opkg(next) {
              ...
              cmd = cmd.concat(
                options.opkg_param ? " " + options.opkg_param : "",
              );
              ...
              luna.send(
                ...
                function (lineObj, next) {
                  ...
                  if (resultValue.match(/FAILED/i)) {
                    ...
                    const errValue =
                      lineObj.details && lineObj.details.reason
                        ? lineObj.details.reason
                        : resultValue
                          ? resultValue
                          : "";
                    next(
          ...
          function (session, next) {
            if (options.opkg) {
              ...
              if (options.session.getDevice().username !== "root") {
                return setImmediate(
                  next,
                  errHndl.getErrMsg("NEED_ROOT_PERMISSION", "opkg remove"),
                );
              }
            }
            setImmediate(next);
            ...
            function _opkg(next) {
              ...
              function __data(data) {
                ...
                if (str.match(/No packages installed or removed/g)) {
                  return next(
                    errHndl.getErrMsg("FAILED_REMOVE_PACKAGE", packageName),
                  );
                } else {
                  middleCb(str.trim());
                }
          ...
          function (next) {
            if (options.opkg) {
              ...
              if (options.session.getDevice().username !== "root") {
                return setImmediate(
                  next,
                  errHndl.getErrMsg("NEED_ROOT_PERMISSION", "opkg list"),
                );
              }
            }
            setImmediate(next);
              ...
              luna.send(
                ...
                function (lineObj, next) {
                  ...
                  if (Array.isArray(resultValue)) {
                    // success: stop
                    if (
                      cliData.getConfig(true).profile &&
                      cliData.getConfig(true).profile !== "signage"
                    ) {
