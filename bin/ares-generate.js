...
let idx;
if (
  (idx = process.argv.indexOf("--list")) !== -1 ||
  (idx = process.argv.indexOf("-l")) !== -1
) {
...
function generate() {
  ...
  if (!options.tmplName) {
    options.tmplName = Object.keys(templates).find(
      (t) => templates[t].default === true,
    );
  }
  ...
  Promise.resolve()
      ...
      const questions = [
        {
          ...
          name: "overwrite",
          message:
            "The directory already exists. The template files in the directory will be replaced. Continue?",
          default: false,
          when: function () {
    ...
    .then(function () {
      ...
      if (!template)
        throw errHndl.getErrMsg("INVALID_VALUE", "TEMPLATE", options.tmplName);
      if (!template.type) {
        return;
      }
      ...
      if (!options.props.length) {
        if (options.query && options.tmplName.match(/(^hosted)/)) {
          ...
          options.query &&
          template.type.match(/(package$|packageinfo$)/)
        ) {
