...
(function () {
  ...
  devicetools.setDefaultDevice = function (name, next) {
    try {
      const resolver =
          this.resolver || (this.resolver = new novacom.Resolver()),
  ...
  devicetools.modifyDeviceInfo = function (options, next) {
    try {
      ...
      if (options[mode].match(/^-/)) {
        ...
        if (
          indexMode != -1 &&
          options.argv.cooked.length > indexMode + 2 &&
          !options.argv.cooked[indexMode + 2].match(/^--[a-z]+$/)
        ) {
      ...
      if (
        typeof inDevice.password !== "undefined" &&
        inDevice.password !== "@DELETE@"
      ) {
  ...
  function replaceDefaultDeviceInfo(inDevice) {
    if (inDevice) {
      ...
      inDevice.description =
        inDevice.description || defaultDeviceInfo.description;
      inDevice.default = inDevice.default || defaultDeviceInfo.default;
    }
  }

  function displayGetKeyGuide(deviceName) {
    const profile = appdata.getConfig().profile;
    let webOSName = "webOS TV",
      guideURL =
        "https://webostv.developer.lge.com/develop/getting-started/developer-mode-app";
    if (profile === "signage") {
      ((webOSName = "Signage"),
        (guideURL =
          "https://webossignage-docs.developer.lge.com/guides/developer-guides/system-feature/developer-mode"));
    }

    const guideSteps =
      "** After adding your device, you'll need to get the key file from your " +
      webOSName +
      "." +
      "\n Follow these steps:" +
      "\n 1. Ensure that the key server button in the Developer Mode app on your " +
      webOSName +
      " is enabled." +
      '\n 2. Run the command "ares-novacom --device ' +
      deviceName +
      ' --getkey" to retrieve the key file from your ' +
      webOSName +
      "." +
