...
const Cli = (function () {
  ...
  function init() {
    const workParentDir = path.resolve(
    ...
    const workDir = this.config.dataDir
        ? path.join(workParentDir, this.config.dataDir)
        : path.join(workParentDir, ".webos"),
      builtinDeviceListFile = path.join(
        builtinFiles.deviceList.dir,
        builtinFiles.deviceList.file,
      ),
    ...
    try {
      ...
      if (result && result.resultValue) {
        ...
        log.verbose(
          "cli-appdata#init()",
          "copy " + builtinDeviceListFile + " to " + userDeviceListFile,
        );
    ...
    if (this.config.simulatorConfig) {
      this.simulatorConfigPath = path.join(
        workDir,
  ...
  CliAppData.prototype = {
    ...
    setDeviceList: function (deviceListData, next) {
      ...
      try {
        ...
        fs.writeFileSync(
          this.deviceListFile,
          JSON.stringify(this.deviceList, null, Number(4)),
        );
      } catch (err) {
        return finish(err, this.deviceList, next);
      }
    ...
    setConfig: function (configData, next) {
      try {
        ...
        fs.writeFileSync(
          this.configFile,
          JSON.stringify(this.config, null, Number(4)) + "\n",
        );
      } catch (err) {
    ...
    setTemplate: function (templateData, next) {
      try {
        ...
        fs.writeFileSync(
          this.templateFile,
          JSON.stringify(this.template, null, Number(4)) + "\n",
        );
      } catch (err) {
    ...
    setEnv: function (envData, next) {
      try {
        ...
        fs.writeFileSync(
          this.envFile,
          JSON.stringify(this.env, null, Number(4)) + "\n",
        );
      } catch (err) {
