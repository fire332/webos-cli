...
(function () {
  ...
  const logLib = {
      ...
      async.series(
        ...
        function (err) {
          if (options.argv.save) {
            ...
            options.argv["id-filter"] &&
            err &&
            err[0].heading === "[ssh exec failure]:"
          ) {
    ...
    readMode: function (options, next) {
      ...
      function _generateCmd(next) {
        ...
        if (options.argv.file) {
          options.cmd =
            "journalctl --file /run/log/journal/" +
            options.logDir +
            "/" +
            options.argv.file;
    ...
    printUnitList: function (options, next) {
      ...
      function _generateCmd(next) {
        ...
        log.verbose(
          "log#printUnitList()#_generateCmd()",
          "options.cmd:" + options.cmd,
        );
        next();
      }
    ...
    checkLogDaemon: function (options, next) {
      ...
      async.series(
                ...
                if (!logDaemonStatus) {
                  return next(
                    errHndl.getErrMsg(
                      ...
                      options.currentDaemon === "journald"
                        ? "pmlogd"
                        : "journald",
                    ),
                  );
        ...
        function (err) {
          if (err) {
            console.log(
              "CLI's current logging daemon : " + options.currentDaemon,
            );
            next(err, null);
          } else {
            next(null, {
              msg:
                "CLI's current logging daemon : " +
                options.currentDaemon +
                "\nThe target's current logging daemon : " +
                options.currentDaemon,
            });
          }
        },
  ...
  function checkUser(options, next) {
    log.info(
      "log#checkUser()",
      "username:",
      options.session.getDevice().username,
    );
  ...
  function generateCmd(options, next) {
    ...
    if (options.currentDaemon === "pmlogd") {
      ...
      if (options.argv["id-filter"]) {
        ...
        const idReg = /^-/;
        if (
          options.argv["id-filter"] === "true" ||
          options.argv["id-filter"].match(idReg)
        ) {
  ...
  function createLogFile(options, next) {
    ...
    if (options.argv.argv.remain.length === 0) {
      savedFilePath = path.resolve(
        createDateFileName(fileNameSeperator, fileExt),
      );
    } else {
      if (path.extname(options.argv.argv.remain[0]) === "") {
        options.argv.argv.remain[0] =
          options.argv.argv.remain[0] + "." + fileExt;
      }
    ...
    log.verbose(
      "log#createLogFile()",
      savedFilePath + " is exist: " + fs.existsSync(savedFilePath),
    );
  }
