...
(function () {
  ...
  const launcher = {
    ...
    launch: function (options, id, params, next, middleCb) {
      ...
      const self = this,
        currentProfile = appdata.getConfig(true).profile,
        hostedAppId =
          currentProfile === "signage"
            ? "com.lg.app.signage.dev"
            : "com.sdk.ares.hostedapp",
        hostedAppIpkName =
          currentProfile === "signage"
            ? "com.sdk.ares.signage.hostedapp"
            : "com.sdk.ares.hostedapp";
      ...
      function _installHostedApp(next) {
        if (options.installMode === "Hosted" && !hostedAppInstalled) {
          const hostedAppUrl = path.join(__dirname, hostedAppIpkName + ".ipk");
          options.appId = id;
        ...
        if (options.installMode === "Hosted") {
          if (options.hostIp) {
            ...
            for (const devName in networkInterfaces) {
              for (
                let index = 0;
                index < networkInterfaces[devName].length;
                index++
              ) {
      ...
      function _runInspector(next) {
        if (options.inspect) {
          ...
          process.on("SIGINT", function () {
            log.verbose(
              "launcher#launch#_runInspector():",
              "SIGINT is detected in Hosted mode",
            );
            next();
          });
        } else {
    ...
    close: function (options, id, params, next) {
      ...
      async.series(
        [_makeSession, _getSessionList, _checkDisplayAffinity, _close],
        function (err, results) {
    ...
    listRunningApp: function (options, next) {
      ...
      options = options || {};
      async.series(
        [_makeSession, _getSessionList, _listRunningApp],
        function (err, results) {
          log.silly(
            "launch#listRunningApp()",
            "err:",
            err,
            ", results:",
            results,
    ...
    launchSimulator: function (options, appDir, params, next, middleCb) {
      ...
      function _checkSimulatorConfig(next) {
        ...
        if (options.simulatorPath) {
          ...
          try {
            fs.writeFileSync(
              configPath,
              JSON.stringify(cofigData, null, Number(4)) + "\n",
            );
          } catch (err) {
            next(err, null);
          }
      ...
      function _checkEnvPath(simulatorDir, next) {
        log.silly(
          "launcher#launchSimulator#_checkEnvPath():",
          "simulatorDir:",
          simulatorDir,
        );
        if (!simulatorDir) {
          sdkenv.getEnvValue("SDK", function (err, sdkHomePath) {
            if (sdkHomePath) {
              middleCb(
                chalk.yellow(
                  "[WARNING] Use <--simulator-path> option instead of environment variable <LG_WEBOS_TV_SDK_HOME>",
                ),
              );
      ...
      function _checkSimulatorDirectory(dirPath, next) {
        log.silly(
          "launcher#launchSimulator#_checkSimulatorDirectory():",
          "dirPath:",
          dirPath,
        );
      ...
      function _findSimulatorVersion(fileList, next) {
        log.silly(
          "launcher#launchSimulator#_findSimulatorVersion():",
          "fileList:",
          fileList,
        );
        let versionList = [];
          ...
          if (fileList[i].indexOf(options.simulatorPrefix) === 0) {
            ...
            if (
              ...
              versionList.push(
                path
                  .parse(filePath)
                  .name.slice(options.simulatorPrefix.length + 1),
              );
            }
          }
        }
        ...
        if (versionList.length === 0) {
          next(
            errHndl.getErrMsg("NOT_EXIST_SIMULATOR", options.simulatorPrefix),
          );
        } else {
          try {
      ...
      function _launchSimulator(next) {
        ...
        if (!fs.existsSync(simulatorPath)) {
          return setImmediate(
            next,
            errHndl.getErrMsg("NOT_EXIST_PATH", simulatorPath),
          );
        }
  ...
  function makeSession(options, next) {
    ...
    if (!options.session) {
      ...
      const printTarget =
        options.printTarget === false ? options.printTarget : true;
  ...
  function checkDisplayAffinity(options, params, next) {
    ...
    if (!options.sessionCall) {
      if (
        params &&
        params.displayAffinity !== undefined &&
        params.displayAffinity !== null
      ) {
        ...
        if (
          options &&
          options.display &&
          Number(options.display) !== params.displayAffinity
        ) {
